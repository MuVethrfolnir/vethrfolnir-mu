 ...
 *
 */
public class RequestLeaveParty extends MuReadPacket {

	@Override
	public void read(MuClient client, ByteBuf buff, Object... params) {
		int partySlot = readC(buff);
		
		GameObject entity = client.getEntity();
		PlayerState entityState = entity.get(PlayerMapping.PlayerState);
		
		if(entityState.getParty() != null) {

			MuParty party = entityState.getParty();
			GameObject target = party.getPartyMember(partySlot);

			boolean isLeader = party.isPartyLeader(target);
			System.out.println("IsLeader: "+isLeader);
			
			if(isLeader)
				party.removePartyMember(entity);
			else
				party.removePartyMember(target);
