 ...
 *
 */
public class RequestAttack extends MuReadPacket {
	...
	public void read(MuClient client, ByteBuf buff, Object... params) {
		...
		// action type and heading are useless here
		
		GameObject entity = entityWorld.getEntitys().get(objId);

		if(entity == null) {
			log.info("Client["+client+"] tryed to attack an non existant target "+objId);
			client.close();
			return;
		}
		...
		if(ap != bp) {
			log.info("Client["+client+"] tryed to attack an target["+objId+"] thats not in the same region."); // packet hack, by remembering the objId
			client.close();
			return;
		}

		CombatEngine.computeSimpleAttack(client.getEntity(), entity);
	}
