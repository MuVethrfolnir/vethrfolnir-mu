 ...
 *
 */
public class RequestPartyInfo extends MuReadPacket {
	
	@Override
	public void read(MuClient client, ByteBuf buff, Object... params) {
		GameObject activeChar = client.getEntity();
		PlayerState state = activeChar.get(PlayerMapping.PlayerState);
		
		if(state.getParty() != null)
			state.getParty().updateUI(false);
	}
